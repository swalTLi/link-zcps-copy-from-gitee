      ...
      >
        ...
        <div>
          ...
          <a-form-model-item label="描述" prop="description">
            <a-textarea
                style="width: 1050px;height:400px;font-size: 20px;"
...
export default {
  ...
  async mounted() {
    await XHR.Axios_$$appointReview_getTeacherApplyRateInfo({
      appointReviewId: this.$route.query.rankReviewId,
    }).then(_ => {
      if (_.data.info.all === 3) {
        this.is_ok = true;
        Cookie.set('is_ok', true);
        this.current = _.data.info.input - 1;
      } else {
        Cookie.set('is_ok', false);
      }

      if ('appointReviewId' in this.$route.query) {
        this.getList();
      } else {
        XHR.Axios_$$appointReview_getTeacherApplyInfo({
          appointReviewId: this.$route.query.rankReviewId,
        }).then(_ => {
          this.baoming = _.data.info.teacherApply.ruleTeacherChooseName;
          this.appointReviewId = _.data.info.teacherApply.appointReviewId;
          this.getList();
        });
      }
    });
  ...
  methods: {
    async getList() {
      this.tableData = [];
      await XHR.Axios_$$appointReview_teacherInput_getTeacherInputInfo({
        page: 1,
        line: 1000,
        rankReviewId: this.$route.query.rankReviewId,
        appointReviewId:
            this.$route.query.appointReviewId || this.appointReviewId,
      }).then(_ => {
        if ('teacherInput' in _.data.info) {
          this.tableData = [_.data.info.teacherInput];
        }
      });
    },
    async openmodal() {
      ...
      this.new_model = {
        ...
        handleOk: async () => {
          console.log('====================================');
          this.new_model.record.appointReviewId =
              this.$route.query.appointReviewId || this.appointReviewId;
          ...
          this.$refs.jtyqForm.validate(valid => {
            if (valid) {
              ...
              XHR.Axios_$$appointReview_teacherInput_create(formData).then(
                  _ => {
                    console.log(_);
    ...
    sh() {
      ...
      XHR.Axios_$$appointReview_teacherInput_upload({
        appointReviewId:
            this.$route.query.appointReviewId || this.appointReviewId,
      }).then(_ => {
        console.log(_.data.info);
    ...
    async deleteRecord(record) {
      await XHR.Axios_$$appointReview_teacherInput_delete({
        id: record.id,
        appointReviewId:
            this.$route.query.appointReviewId || this.appointReviewId,
      }).then(_ => {
    ...
    beforeUpload(file) {
      ...
      if (!isJpgOrPng) {
        this.$message.error(
            '当前文件为' + file.type + '类型，只允许上传 pdf、doc、dox、zip类型文件，并且限制大小25MB!'
        );
      }
