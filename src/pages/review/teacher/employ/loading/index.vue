  ...
  <a-card :body-style="{ padding: '24px 32px' }" :bordered="false">
    ...
    <div style="margin:50px 15%;" v-if="!loading">
      <a-steps :current="current" class="asteps">
        <a-step
          title="报名"
          @click="nextPage('/employ/teacher-employ-signup', 0)"
        >
          <span slot="description" v-if="current === 0"
            ><a @click="nextPage('/employ/teacher-employ-signup', 0)"
              >点击报名</a
            >
          </span></a-step
        >
        <!-- <a-step
          title="填报工作量条件"
          @click="nextPage('/employ/teacher-employ-workload', 1)"
        >
          <span slot="description" v-if="current === 1"
            ><a @click="nextPage('/employ/teacher-employ-workload', 1)">点击填报</a>
          </span>
        </a-step> -->
        <a-step
          title="填报业绩条件"
          @click="nextPage('/employ/teacher-employ-condition', 1)"
        >
          <span slot="description" v-if="current === 1"
            ><a @click="nextPage('/employ/teacher-employ-condition', 1)"
              >点击填报</a
            >
          </span>
          <span slot="description" v-else-if="current > 1"
            ><a @click="nextPage('/employ/teacher-employ-condition', 2)">查看</a>
          </span>
        </a-step>
        <a-step title="审核" />
        <a-step title="审核结束" />
