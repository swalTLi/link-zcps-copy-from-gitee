  ...
  <a-card :body-style="{ padding: '24px 32px' }" :bordered="false">
    ...
    <div style="margin:50px 15%;" v-if="!loading">
      <a-steps :current="current" class="asteps">
        <a-step title="报名" @click="nextPage('/post/teacher-post-signup', 0)">
        ...
        <a-step
          title="填报工作量条件"
          @click="nextPage('/post/teacher-post-workload', 1)"
        >
          <span slot="description" v-if="current === 1"
            ><a @click="nextPage('/post/teacher-post-workload', 1)">点击填报</a>
          </span>
          <span slot="description" v-else-if="current <= 4 && current > 1"
            ><a @click="goto('/post/teacher-post-workload', 1)">查看</a>
          </span>
        </a-step>
        <a-step
          title="填报业绩条件"
          @click="nextPage('/post/teacher-post-condition', 2)"
        >
          <span slot="description" v-if="current === 2"
            ><a @click="nextPage('/post/teacher-post-condition', 2)"
              >点击填报</a
            >
          </span>
          <span slot="description" v-else-if="current <= 4 && current > 2"
            ><a @click="goto('/post/teacher-post-condition', 1)">查看</a>
          </span>
        </a-step>
        <a-step title="审核" />
        <a-step title="审核结束" />
