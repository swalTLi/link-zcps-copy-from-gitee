    ...
    <div>
      <a-card style="width:50%;margin:auto;margin-top:50px;margin-bottom:50px;">
        ...
        >
          ...
          <a-divider />
          <div class="info">
            <label>合计:</label><label>{{ stepData['jxgzl']['count'] }}</label>
          </div>
        </div>
        <div
          style="width:50%;margin:auto;font-size:20px;"
        ...
        >
          ...
          <a-divider />
          <div class="info">
            <label>合计:</label><label>{{ stepData['kygzl']['count'] }}</label>
          </div>
        </div>
        <div
          style="width:50%;margin:auto;font-size:20px;"
        ...
        >
          ...
          <a-divider />
          <div class="info">
            <label>合计:</label><label>{{ stepData['jygzl']['count'] }}</label>
          </div>
        </div>
        <div
          style="width:50%;margin:auto;font-size:20px;"
        ...
        >
          ...
          <a-divider />
          <div class="info">
            <label>合计:</label><label>{{ stepData['cxgzl']['count'] }}</label>
          </div>
        </div>
        <div
          style="width:50%;margin:auto;font-size:20px;"
        ...
        >
          ...
          <div class="info">
            <label>工作量合计:</label><label>{{ stepData['count'] }}</label>
          </div>
        </div>
      </a-card>
      <div
        v-if="stepData.length !== 0"
        style="margin:auto;width:300px;display:flex;justify-content:space-around;"
      >
        <a-button
          type="primary"
          size="large"
        ...
        <a-button
          type="primary"
          size="large"
          @click="goto('/post/teacher-post-condition', { ...$route.query })"
          v-if="currentStep === 4 && is_ok && state"
          >填报业绩</a-button
        >
        <a-button
          v-if="is_ok && !state"
  ...
  methods: {
    async getList() {
      await XHR.Axios_$$positionAppointment_apply_getRateInfo({
        ...
        if (_.data.info.workload === 2) {
          this.currentStep = 4;
          this.is_ok = true;
          this.state = true;
        }
        if (_.data.info.workload === 4) {
          this.currentStep = 4;
          this.is_ok = true;
          this.state = false;
        }
      });
      await XHR.Axios_$$positionAppointment_getTeacherRankWorkload({
        positionAppointmentReviewId: this.$route.query
          .positionAppointmentReviewId,
      }).then(_ => {
        console.log(_.data.info);
        if (_.data.info.length === 0) {
          XHR.Axios_$$positionAppointment_gatherTeacherRankWorkload({
            positionAppointmentReviewId: this.$route.query
              .positionAppointmentReviewId,
          }).then(__ => {
    ...
    setTeacherRankWorkload() {
      XHR.Axios_$$positionAppointment_setTeacherRankWorkload({
        positionAppointmentReviewId: this.$route.query
          .positionAppointmentReviewId,
      }).then(_ => {
        console.log(_);
