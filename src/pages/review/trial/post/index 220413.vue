  ...
  <a-card :bordered="false">
    ...
    >
      ...
      <div style="margin-left: 50px;" class="label">
        <label style="line-height:30px;">单位：</label>
        ...
        ><br />
        <label>业绩描述：</label> <span>{{ record.description }}</span
        ><br />
      </div>
...
export default {
  ...
  data() {
    ...
    const COLUMNS = [
      ...
      //     const teacherName = this.dataSource[index - 1]
      //       ? this.dataSource[index - 1].teacherName
      //       : "";
  ...
  methods: {
    async getList() {
      ...
      await XHR.Axios_$$positionAppointmentReview_searchTeacherUploadList({
        ...
        positionAppointmentId: this.$route.query.positionAppointmentId,
    ...
    async SEE(record) {
      await XHR.Axios_$$positionAppointmentReview_searchTeacherUploadInfo({
    ...
    async TG(record) {
      ...
      console.log(info);
      await XHR.Axios_$$positionAppointmentReview_checkTeacherFill({
        id: record.teacherInputId,
        ...info,
      }).then(_ => {
        console.log(_);
