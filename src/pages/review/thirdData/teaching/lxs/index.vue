...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      XHR.Axios_$$thirdPartyData_jxgzl_data_getList(this.info).then(_ => {
    ...
    handleEdit(query) {
      console.log(query);
      // 调用接口
      XHR.Axios_$$thirdPartyData_jxgzl_data_edit({
        operate: 2,
        type: this.info.type,
        id: query.id,
    ...
    showModal() {
      var date = new Date();
      this.yearUpload = date.getFullYear();
    ...
    async handleUpload() {
      ...
      formData.append(`info[year]`, this.yearUpload);
      if (this.yearUpload) {
        await XHR.Axios_$$thirdPartyData_jxgzl_upload(formData).then(_ => {
          console.log(_);
