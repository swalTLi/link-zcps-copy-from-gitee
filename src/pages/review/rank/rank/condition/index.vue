      ...
      <div>
        ...
        >
          ...
          <div slot="action" slot-scope="{ record, index }">
            ...
            >
              <a-icon :component="svg_condition" /> 条件详情
            </a>
            <a
              style="margin-right: 8px"
              @click="opendetailmodal('AC', record, index)"
            >
              <a-icon :component="svg_performanceConditions" /> 业绩条件
            </a>
            <a-popconfirm
              title="确认删除？"
              ok-text="删除"
...
import CI from "@/pages/review/components/CIModel";
import AC from "@/pages/review/components/ACModel";
// 数据配置
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      XHR.Axios_$$searchPerformanceCriteriaList(this.info).then(_ => {
    ...
    handleEdit(query) {
      console.log(query);
      // 调用接口
      XHR.Axios_$$addPerformanceCriteria({
        operate: 2,
        type: this.info.type,
        id: query.id,
        ...query,
    ...
    async opendetailmodal(option, record = {}, index) {
      ...
      if (option === "Edit") {
        ...
        await XHR.Axios_$$performanceCriteria_searchRegisterRequirement({
          performanceCriteriaId: record.id,
        }).then(e => {
          const type =
          ...
          this.BM_model = {
            ...
            handleOk: async () => {
              await XHR.Axios_$$performanceCriteria_editRegisterRequirement({
    ...
    // 删除指定数据
    deleteRecord(query) {
      XHR.Axios_$$deletePerformanceCriteria({
        operate: 3,
        id: query.id,
        type: this.info.type,
