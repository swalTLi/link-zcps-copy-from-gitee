      ...
      <div>
        ...
        >
          ...
          <div slot="action" slot-scope="{ record }">
            <a style="margin-right: 8px" @click="goto('/achievement', record,  1)"
               >
...
export default {
  ...
  methods: {
    ...
    goto(path, query) {
      console.log(path)
      console.log(query)
      alert(query.teacherId)
      this.$router.push({
        path,
        query: { ...query, rankReviewId:this.$route.query.rankReviewId},
      });
     /* if (!query.open) {
    ...
    async getList() {
      ...
      // 默认查第一个条件
      await XHR.Axios_$$searchTeacherRankApplyList({
        page: 1,
        line: 1000,
        rankReviewId: this.$route.query.rankReviewId,
    ...
    deleteRecord(record) {
      console.log(record);
      XHR.Axios_$$deleteTeacherRankApply({ id: record.id }).then(() =>
        this.getList()
      );
      return this.getList();
