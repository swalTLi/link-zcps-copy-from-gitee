    ...
    <a-card>
      ...
      <div>
        ...
        >
          ...
          <div slot="public" slot-scope="record">
            <a-tag
                    :color="
                record.record['public'] === 0
                  ? 'red'
                  : record.record['public'] === 1
                  ? 'green'
                  : 'grey'
              "
            >
              {{ getLabel(record.record, "public") }}
            </a-tag>
          </div>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      XHR.Axios_$$positionAppointmentReview_getPositionAppointmentReviewList(
        this.info
      ).then(_ => {
    ...
    handleEdit(query) {
      console.log(query);
      // 调用接口
      XHR.Axios_$$positionAppointmentReview_edit({
        id:this.id,
        ...query
      }).then(_ => {
    ...
    handleAdd(query) {
      console.log(query);
      // 调用接口
      XHR.Axios_$$positionAppointmentReview_edit(query).then(_ => {
        this.$refs["DetailModel"].handleCallBack(_.data);
      });
    },
    // 删除
    ...
    deleteRecord(record) {
      console.log(record);
      XHR.Axios_$$positionAppointmentReview_delete({ id: record.id }).then(() =>
        this.getList()
      );
      return this.getList();
    },
