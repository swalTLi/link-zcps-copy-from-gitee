    ...
    <a-card>
            ...
            <a-row>
              ...
              <a-col :md="12" :sm="20">
                ...
                >
                  <a-select
                    placeholder="请选择"
                    v-model="filterForm.performanceCriteriaid"
                  >
                    <a-select-option
                      :value="item.id"
                      v-for="item in performanceCriteriaList"
                      :key="item.id"
                      @click="selectFilter"
              ...
              <a-col :md="8" :sm="24">
                <a-form-item
                        ...
                        :wrapperCol="{ span: 18, offset: 1 }"
                >
                  <a-button type="primary" @click="calculationreview()">
                    计算评审1
                  </a-button>
                </a-form-item>
              </a-col>
      ...
      <div>
        <div style="display: flex;justify-content: space-between;">
          ...
          <a-pagination
            style="right:0px;margin-bottom:20px;"
        ...
        >
            ...
            <a-tag
              :color="
                record.record['open'] === 0
                  ? 'red'
                  : record.record['open'] === 1
                  ? 'green'
                  : 'grey'
              "
            >
              {{ getLabel(record.record, "open") }}
            </a-tag>
          </div>
          ...
          <div slot="action" slot-scope="{ record }">
            <a style="margin-right: 8px" @click="getInfo(record)">
...
export default {
  ...
  //   // this.searchPerformanceCriteriaList();
  //   this.getList();
  // },
  async activated() {
    // await XHR.Axios_$$positionAppointment_require_searchList({
    //   id: this.$route.query.rankReviewId,
    //   page: 0,
    //   line: 10000,
    // }).then(_ => {
    //   console.log(_);
    //   this.performanceCriteriaList = _.data.info.list;
    // });
    await this.searchPerformanceCriteriaList();
    await this.getList();
  },
  methods: {
    async getInfo(record) {
      console.log(record);
      var info;
      await XHR.Axios_$$positionAppointment_apply_getInfo({
        sid: record.teacherSid,
        positionAppointmentReviewId: this.$route.query.rankReviewId,
      }).then(_ => {
        console.log(_.data.info);
        info = _.data.info;
      });
      await this.opendetailmodal("Info", { ...record, ...info.teacherApply });
    },
    // 读取下拉框数据
    ...
    async searchPerformanceCriteriaList() {
      // 查列表
      await XHR.Axios_$$positionAppointment_searchList({
        page: 0,
        line: 10000,
      }).then(_ => {
        console.log(_.data.info);
        this.performanceCriteriaList = _.data.info.list;
        this.filterForm.performanceCriteriaid = _.data.info.list[0].id;
      });
    },
    async getList() {
      ...
      // 默认查第一个条件
      await XHR.Axios_$$positionAppointment_apply_searchList({
        page: 1,
        line: 1000,
        positionAppointmentReviewId: this.$route.query.rankReviewId,
        positionAppointmentId: this.filterForm.performanceCriteriaid,
      }).then(_ => {
    ...
    //计算评审按钮
    // calculationreview(){
    //   XHR.Axios_$$calculateRankResul({
    //     rankReviewId:this.$route.query.rankReviewId,
    //     performanceCriteriaId:this.filterForm.performanceCriteriaid,
    //   }).then(() => {
    //     this.getList();
    //   });
    // },
    calculationreview(){
      XHR.Axios_$$calculatePositionAppointmentResult({
        positionAppointmentReviewId:this.$route.query.rankReviewId,
        positionAppointmentId:this.filterForm.performanceCriteriaid,
      }).then(() => {
        this.getList();
    ...
    handleEdit(query) {
      console.log(query);
      // 调用接口
      XHR.Axios_$$rankReview_edit(query).then(_ => {
    ...
    deleteRecord(record) {
      console.log(record);
      XHR.Axios_$$positionAppointment_apply_delete({ id: record.id }).then(() =>
        this.getList()
      );
      return this.getList();
