...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      XHR.Axios_$$appointReview_getList(this.info).then(_ => {
    ...
    handleAdd(query) {
      console.log(query);
      // 调用接口
      XHR.Axios_$$appointReview_add(query).then(_ => {
        this.$refs["DetailModel"].handleCallBack(_.data);
      });
    },
    // 删除
    ...
    deleteRecord(record) {
      console.log(record);
      XHR.Axios_$$appointReview_delete({ id: record.id }).then(() =>
        this.getList()
      );
      return this.getList();
    },
