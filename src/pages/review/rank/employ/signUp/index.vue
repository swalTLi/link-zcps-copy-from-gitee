                ...
                >
                  ...
                  >
                    <a-select-option
                      ...
                      :key="item.id"
                      @click="selectFilter"
                      >{{ item.teacherChooseName }}</a-select-option
                    >
                  </a-select>
                </a-form-item>
              </a-col>
      ...
      <div>
        <div style="display: flex;justify-content: space-between;">
          <a-space class="operator" align="baseline">
            <button>{{ this.filterForm.performanceCriteriaid }}</button>
            <ImageButton
              v-if="0"
              :src="svg_create"
...
export default {
  ...
  methods: {
    async getInfo(record) {
      console.log(record);
      var info;
      await XHR.Axios_$$appointReview_getTeacherApplyInfo({
        appointReviewId: this.$route.query.rankReviewId,
        sid: record.teacherSid,
      }).then(_ => {
        info = _.data.info;
      });
      console.log(info.teacherApply);
    ...
    async searchPerformanceCriteriaList() {
      // 查列表
      await XHR.Axios_$$appointReview_rule_searchList({
        page: 0,
        line: 10000,
      }).then(_ => {
    ...
    async getList() {
      ...
      // 默认查第一个条件
      await XHR.Axios_$$appointReview_searchTeacherApplyList({
        page: 1,
        line: 1000,
        appointReviewId: this.$route.query.rankReviewId,
        ruleId: this.filterForm.performanceCriteriaid,
      }).then(_ => {
    ...
    deleteRecord(record) {
      console.log(record);
      XHR.Axios_$$appointReview_deleteTeacherApply({
        id: record.id,
        appointReviewId: this.$route.query.rankReviewId,
      }).then(() => this.getList());
      return this.getList();
