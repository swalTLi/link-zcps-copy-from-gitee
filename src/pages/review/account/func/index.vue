    ...
    <a-card>
      ...
      <div>
        ...
        >
          ...
          <div slot="functionName" slot-scope="{ text }">
            {{ text }}
          </div>
...
export default {
  ...
  methods: {
    ...
    handleEdit(query) {
      console.log(query);
      // 调用接口
      XHR.Axios_$$addFunctionAdmin(query).then(_ => {
    ...
    // 删除指定数据
    deleteRecord(record) {
      XHR.Axios_$$deleteUser({ ...record, type: 3 }).then(() => this.getList());
      return this.getList();
    ...
    getFunctionId(text) {
      var name = "";
      try {
        name = this.columns[3].selects.find(_ => _.id == text).name;
      } catch (e) {
        console.log(e);
        name = text;
      }
