    ...
    <a-card>
      ...
      <div>
        ...
        >
          ...
          <!-- <div slot="secondarySectorId" slot-scope="{ text }">
            {{ columns[3].selects.find(_ => _.id == text).name || '无' }}
          </div> -->
          <div slot="secondarySectorName" slot-scope="{ text }">
            {{ text }}
          </div>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      XHR.Axios_$$getUserList(this.info).then(_ => {
    ...
    handleEdit(query) {
      console.log(query);
      // 调用接口
      XHR.Axios_$$addSecondarySectorAdmin(query).then(_ => {
    ...
    // 删除指定数据
    deleteRecord(record) {
      XHR.Axios_$$deleteUser({ ...record, type: 4 }).then(() => this.getList());
      return this.getList();
