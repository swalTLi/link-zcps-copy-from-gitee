...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      XHR.Axios_$$searchFunctionDepartmentList(this.info).then(_ => {
    ...
    handleEdit(query) {
      console.log(query);
      // 调用接口
      XHR.Axios_$$addFunctionDepartment(query).then(_ => {
    ...
    // 删除指定数据
    deleteRecord(record) {
      XHR.Axios_$$deleteFunctionDepartment(record).then(() => this.getList());
      return this.getList();
