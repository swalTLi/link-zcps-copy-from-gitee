...
// API接口
import { Axios_$$getUserList } from "@/services/interface";
// svg组件
...
export default {
  ...
  data() {
    return {
      ...
      info: {
        ...
        // 院系账号 二级部门
        type: 4,
        secondarySectorNumber: undefined,
      },
      // 自定义编辑框
      detail_model_data: {
        visible: false,
  ...
  methods: {
    getList(query) {
      Axios_$$getUserList(query).then(_ => {
    ...
    opendetailmodal(option, record) {
      ...
      if (option === "Edit") {
        this.detail_model_data = {
          visible: true,
          tableTitle: `编辑`, userType: this.info.type,
          width: 800,
          type: option,
        ...
        this.detail_model_data = {
          visible: true,
          tableTitle: `用户详情`,           userType: this.info.type,
          width: 1000,
          type: option,
          columns: this.columns,
          record: record,
